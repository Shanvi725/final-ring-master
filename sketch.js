...
function draw() {
  ...
  if (gameState === PLAY){
    ...
    if(ring.isTouching(birdG) && (keyDown("space"))){
      for(var i=0;i<birdG.length;i++){
        if(birdG.get(i).isTouching(ring)){
        birdG.get(i).destroy();
        }
      }
      score = score + 10;
    ...
    if(ring.isTouching(birdG2) && (keyDown("space"))){
      for(var i=0;i<birdG2.length;i++){
        if(birdG2.get(i).isTouching(ring)){
        birdG2.get(i).destroy();
        }
      }
      score = score + 10;
    }
    ...
    if (line.isTouching(birdG)){
      ...
      for(var i=0;i<birdG.length;i++){
        if(birdG.get(i).isTouching(line)){
        birdG.get(i).destroy();
        }
      }
    count = count + 1;
    ...
    if (line2.isTouching(birdG2)){
      ...
      for(var i=0;i<birdG2.length;i++){
        if(birdG2.get(i).isTouching(line2)){
        birdG2.get(i).destroy();
        }
      }
    count = count + 1;
